...
class MemberTest {
    ...
    fun `given two trustly accounts when a notification from the old account arrives, expect that only the old account will be updated`() {
        ...
        fixture
            ...
            .expectState { member ->
                assertThat(member.trustlyAccountsBasedOnHedvigOrderId.size).isEqualTo(2)
                assertThat(member.trustlyAccountsBasedOnHedvigOrderId[HEDVIG_ORDER_ID_ONE]!!.directDebitStatus).isEqualTo(
                    DirectDebitStatus.DISCONNECTED
                )
                assertThat(member.trustlyAccountsBasedOnHedvigOrderId[HEDVIG_ORDER_ID_ONE]!!.accountId).isEqualTo(
                    TRUSTLY_ACCOUNT_ID
                )

                assertThat(member.trustlyAccountsBasedOnHedvigOrderId[HEDVIG_ORDER_ID_TWO]!!.directDebitStatus).isEqualTo(
                    DirectDebitStatus.CONNECTED
                )
                assertThat(member.trustlyAccountsBasedOnHedvigOrderId[HEDVIG_ORDER_ID_TWO]!!.accountId).isEqualTo(
    ...
    @Test
    fun `given three trustly accounts when a "disconnected" notification for the latest account arrives, expect that only the latest account will be updated`() {
        val secondTrustlyAccountId = "secondTrustlyAccountId"
        ...
        fixture
            .given(
                ...
                makeTrustlyAccountCreatedEvent(MEMBER_ID_ONE, secondTrustlyAccountId, HEDVIG_ORDER_ID_TWO),
                makeDirectDebitConnectedEvent(MEMBER_ID_ONE, secondTrustlyAccountId, HEDVIG_ORDER_ID_TWO),
                makeTrustlyAccountCreatedEvent(MEMBER_ID_ONE, latestTrusltyAccountId, HEDVIG_ORDER_ID_THREE),
                makeDirectDebitConnectedEvent(MEMBER_ID_ONE, latestTrusltyAccountId, HEDVIG_ORDER_ID_THREE)
            )
            .`when`(
                UpdateTrustlyAccountCommand(
                    memberId = MEMBER_ID_ONE,
                    hedvigOrderId = HEDVIG_ORDER_ID_THREE,
            ...
            .expectEvents(
                ...
                DirectDebitDisconnectedEvent(
                    MEMBER_ID_ONE,
                    HEDVIG_ORDER_ID_THREE.toString(),
                    latestTrusltyAccountId
                )
            )
            .expectState { member ->
                assertThat(member.trustlyAccountsBasedOnHedvigOrderId.size).isEqualTo(3)

                assertThat(member.trustlyAccountsBasedOnHedvigOrderId[HEDVIG_ORDER_ID_ONE]!!.accountId).isEqualTo(TRUSTLY_ACCOUNT_ID)
                assertThat(member.trustlyAccountsBasedOnHedvigOrderId[HEDVIG_ORDER_ID_ONE]!!.directDebitStatus).isEqualTo(DirectDebitStatus.CONNECTED)

                assertThat(member.trustlyAccountsBasedOnHedvigOrderId[HEDVIG_ORDER_ID_TWO]!!.accountId).isEqualTo(secondTrustlyAccountId)
                assertThat(member.trustlyAccountsBasedOnHedvigOrderId[HEDVIG_ORDER_ID_TWO]!!.directDebitStatus).isEqualTo(
                    DirectDebitStatus.CONNECTED
                )

                assertThat(member.trustlyAccountsBasedOnHedvigOrderId[HEDVIG_ORDER_ID_THREE]!!.accountId).isEqualTo(latestTrusltyAccountId)
                assertThat(member.trustlyAccountsBasedOnHedvigOrderId[HEDVIG_ORDER_ID_THREE]!!.directDebitStatus).isEqualTo(DirectDebitStatus.DISCONNECTED)
                assertThat(member.latestHedvigOrderId).isEqualTo(HEDVIG_ORDER_ID_THREE)
