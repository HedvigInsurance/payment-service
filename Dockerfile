...
COPY --from=build /payment-service/target /payment-service/target
COPY --from=build /trustly-client/target /trustly-client/target
RUN mvn test
...
COPY --from=build /payment-service/target/payment-service-0.0.1-SNAPSHOT.jar /
